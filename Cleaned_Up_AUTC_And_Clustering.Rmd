...
```{r, clustering_time_10}
DissimMat <- CalcABTC

pamAUTCMaxK <- pam(DissimMat, diss = T, k = subMaxK10)
ClusterMaxK <- pamAUTCMaxK$clustering
DissimMatMaxK <- DissimMat[names(sort(ClusterMaxK)), names(sort(ClusterMaxK))]
MaxKdissplot <- ggdissplot(as.dist(DissimMatMaxK), labels = sort(ClusterMaxK), method = NA, cluster_labels = T, cluster_lines = T, diag = F) + scale_fill_gradient(low = "midnightblue", high = "white", na.value = "grey75") + ggtitle("Individual Model Clustering, 10% of sample analized for clustering")

DissimMat <- CalcABTC
pamAUTCWholeK <- pam(DissimMat, diss = T, k = subWholeK10)
ClusterWholeK <- pamAUTCWholeK$clustering
DissimMatWholeK <- DissimMat[names(sort(ClusterWholeK)), names(sort(ClusterWholeK))]
WholeKdissplot <- ggdissplot(as.dist(DissimMatWholeK), labels = sort(ClusterWholeK), method = NA, cluster_labels = T, cluster_lines = T, diag = F) + scale_fill_gradient(low = "midnightblue", high = "white", na.value = "grey75") + ggtitle("AllxAll Clustering, 10% of sample analized for clustering")
MaxKdissplot
WholeKdissplot
```

```{r, clustering_time_20}
DissimMat <- CalcABTC

pamAUTCMaxK <- pam(DissimMat, diss = T, k = subMaxK20)
ClusterMaxK <- pamAUTCMaxK$clustering
DissimMatMaxK <- DissimMat[names(sort(ClusterMaxK)), names(sort(ClusterMaxK))]
MaxKdissplot <- ggdissplot(as.dist(DissimMatMaxK), labels = sort(ClusterMaxK), method = NA, cluster_labels = T, cluster_lines = T, diag = F) + scale_fill_gradient(low = "midnightblue", high = "white", na.value = "grey75") + ggtitle("Individual Model Clustering, 20% of sample analized for clustering")

DissimMat <- CalcABTC
pamAUTCWholeK <- pam(DissimMat, diss = T, k = subWholeK20)
ClusterWholeK <- pamAUTCWholeK$clustering
DissimMatWholeK <- DissimMat[names(sort(ClusterWholeK)), names(sort(ClusterWholeK))]
WholeKdissplot <- ggdissplot(as.dist(DissimMatWholeK), labels = sort(ClusterWholeK), method = NA, cluster_labels = T, cluster_lines = T, diag = F) + scale_fill_gradient(low = "midnightblue", high = "white", na.value = "grey75") + ggtitle("AllxAll Clustering, 20% of sample analized for clustering")
MaxKdissplot
WholeKdissplot
```

```{r, clustering_time_30}
DissimMat <- CalcABTC
...
ggdendrogram(dendro) + ggtitle("Signature Behaviours according to Intra-Motif Clustering, 30% subsample for Clustering") + theme(plot.title = element_text(size = 32))
...
rownames(clustermedoids) <- colnames(clustermedoids)
dendro <- hclust(as.dist(clustermedoids), method = "average")
ggdendrogram(dendro) + ggtitle("Signature Behaviours according to AllxAll Clustering, 30% subsample for Clustering") + theme(plot.title = element_text(size = 32))
```
...
```{r graphing clusters by motif origin}
tempytempy <- function(data, ClusNum, Cluster, title){
  Partial = tidyr::gather(data[, names(Cluster[which(Cluster == ClusNum)])])
  Partial$step <- rep(seq(from = 0, to = 0.01, by = 1/60000), ncol(data[, names(Cluster[which(Cluster == ClusNum)])]))
  Partial$motif <- str_extract(Partial$key, regex("[A-Z]+v?[1-9]?"))
  plots <- list()
  for (i in unique(Partial$motif)) {
    b <- ggplot(data = Partial[Partial$motif == i,], aes(x = step, y = value, col = key)) + geom_line(size = 1.5) + ylab(expression(paste("CoRa"[mu[Y]%in%theta], "("*mu[Y],")"))) + ggtitle(paste0("Cluster ", ClusNum, " members from motif ", i)) + theme(plot.title = element_text(size = 32), axis.title.y = element_text(size = 32), axis.title.x = element_text(size = 32)) + theme(axis.text = element_text(size = 20)) + theme(legend.position = "none") + ylim(0, 1)
    plots[[i]] = b
  }
  return(plots)
}

plots <- tempytempy(CoRas, 5, ClusterWholeK)
