...
params = CSV.File("InputFiles\\ARGS_ATFv2_Par_Test_Fig2B.csv")	# Perturbation details
include(string("InputFiles\\ARGS_",iARG.mm,"_Pert_",iARG.ex,".jl"))	# Core parameters
keys = (:g, :mY, :gY, :mU, :gU, :mW, :gW, :e0, :eP, :eM, :mUs);
x0 = zeros(length(mm.odeFB.syms));
for i in 1:params.rows
    p = Dict();
    for h in 1:params.cols
        push!(p, keys[h] => params[i][h])
    end
    p0 = copy(p)
    if(iARG.an == "ExSSs")
        ...
        open(string("OUT_ExSSs_",iARG.mm,"_",iARG.ex,"_",iARG.pp,"_",iARG.ax,"_Row", i,".txt"), "w") do outfile1
            ###This generates the headers for our data output, we're preparing the file beforehand
            writedlm(outfile1, [vcat(iARG.ax,[string("FbR_",j) for j in mm.odeFB.syms],[string("FbD_",j) for j in mm.odeFB.syms],[string("NfR_",j) for j in mm.odeNF.syms],[string("NfD_",j) for j in mm.odeNF.syms],string("CoRa(",iARG.pp,")"))],'\t');
