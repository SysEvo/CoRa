...
write.csv(ATF_generate(1250, 6), file = "ARGS_ATFv1_Mass_Par_1250Set2.csv", row.names = FALSE)
```
```{r, Creating_infile_BNF}
set.seed(260399)
setwd("C:/Users/ese_1/OneDrive/Documentos/CoRa/InputFiles/") #Cambiar a carpeta relevante dependiendo de computadora
write.csv(BNF_generate(1250, 6), file = "ARGS_BNFv1_Mass_Par_1250Set2.csv", row.names = FALSE)
```

```{r, Creating_infile_BMF2}
set.seed(160396)
setwd("C:/Users/ese_1/OneDrive/Documentos/CoRa/InputFiles/") #Cambiar a carpeta relevante dependiendo de computadora
write.csv(BMF2_generate(1250, 6), file = "ARGS_BMFv2_Mass_Par_1250Set2.csv", row.names = FALSE)
```

```{r, Creating_infile_BMF1}
set.seed(160396)
setwd("C:/Users/ese_1/OneDrive/Documentos/CoRa/InputFiles/") #Cambiar a carpeta relevante dependiendo de computadora
write.csv(BMF1_generate(1250, 6), file = "ARGS_BMFv1_Mass_Par_1250Set2.csv", row.names = FALSE)
```
```{r, Creating_infile_FAD}
set.seed(230599)
setwd("C:/Users/ese_1/OneDrive/Documentos/CoRa/InputFiles/") #Cambiar a carpeta relevante dependiendo de computadora
write.csv(FAD_generate(1250, 6), file = "ARGS_FADv2_Mass_Par_1250Set2.csv", row.names = FALSE)
```
...
a <- ggplot(data = Graphing, aes(x = step, y = value, col = key)) + geom_line(size = 1.5) + theme(legend.position = "none") + ylab(expression(paste("CoRa"[theta], "(Y, "*mu[Y],")"))) + ylim(0, 1) + ggtitle("Ejemplos de CoRa lines") + theme(plot.title = element_text(size = 32), axis.title.y = element_text(size = 32), axis.title.x = element_text(size = 32)) + theme(axis.text = element_text(size = 20)) + ylim(0, 1)
a
```
